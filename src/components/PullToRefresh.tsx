...
export default function PullToRefresh({ onRefresh, children }: PullToRefreshProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`flex items-center gap-2 text-sm text-[var(--text-muted)] ${refreshing ? 'animate-pulse' : ''}`}>
          <div
          ...
          />
          <span>{refreshing ? '새로고침 중...' : pullDistance > threshold ? '놓으면 새로고침' : '당겨서 새로고침'}</span>
        </div>
      </div>
      {children}
    </div>
